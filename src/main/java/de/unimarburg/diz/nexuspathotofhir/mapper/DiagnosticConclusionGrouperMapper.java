...
public class DiagnosticConclusionGrouperMapper extends ToFhirMapper {
  ...
  public Observation map(PathoInputBase inputBase) {
    ...
    // Add identifier
      ...
      pathoFindingGrouper.setMeta(new Meta()
      ...
      Identifier identifierPathoFinding =  IdentifierAndReferenceUtil.getIdentifier(
          input,
          PathologyIdentifierResourceType.PATHO_FINDING,
          fhirProperties.getSystems().getDiagnosticFindingId(), input.getBefundtyp(), input.getBefundID(), "DIAGNOSE_CONCLUSION");
      hasMembers.add(IdentifierAndReferenceUtil.getReferenceTo("Observation", identifierPathoFinding));
      ...
      Identifier idPathoFindingGrouperMicro =  IdentifierAndReferenceUtil.getIdentifier(
          ...
          fhirProperties.getSystems().getDiagnosticFindingGrouperId(), "-", input.getBefundtyp(), input.getBefundID());
      derievedFrom.add(IdentifierAndReferenceUtil.getReferenceTo("Observation", idPathoFindingGrouperMacro));
      derievedFrom.add(IdentifierAndReferenceUtil.getReferenceTo("Observation", idPathoFindingGrouperMicro));
